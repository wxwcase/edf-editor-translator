...
public abstract class AbstractTranslatorFactory {
  ...
  /**
   * Get signal location from scored event
   * Assume that readMapFile(mapFile) method is called
   * Algorithm:
   *   1. check whether annLocation is one of the EDF file's signal labels
   *   1.1 if true, then return the annLocation
   *   1.2 if false, then check if the signal name list from the mapping file match the EDF signal
   *      names
   *   1.3 if true, return the matched one
   *   1.4 if all of above is false, choose the first EDF signal name as default channel
   * @param scoredEvent event element
   * @param annLocation the signal location recored in the annotation file
   * @return signal location
   */
  public abstract String getSignalLocationFromEvent(Element scoredEvent, String annLocation);
	...
	private void getSignalLabels(RandomAccessFile raf, int signalCount) throws IOException {
	 ...
	 * @param raf the RandomAccessFile of corresponding EDF file
	 * @param signalNumber the 'signalNumber'th signal
